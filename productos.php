...
if("GET_ALL"==$action){
    $db_data=array();
    $sql= "SELECT ce_product.product_id, ce_product.model, ce_product.image, ce_product.price, ce_product.quantity,ce_product.length, ce_product.width, ce_product.height, ce_product_description.name, ce_product_description.description FROM ce_product join ce_product_description on ce_product.product_id=ce_product_description.product_id ORDER BY RAND() LIMIT 10;";
...
if("GET_POPUS"==$action){
    $result=$conn->query("SELECT ce_product.product_id, ce_product.model, ce_product.image, ce_product.price, ce_product.quantity, ce_product_description.name from ce_product join ce_product_description on ce_product.product_id=ce_product_description.product_id order by viewed desc limit 10 ;");
    if($result ->num_rows>  0){
        while($row = $result->fetch_assoc()){
            $db_data[]=$row;
        }
