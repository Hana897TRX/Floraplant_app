...
if("GET_ALL"==$action){
    $db_data=array();
    $sql= "SELECT ce_product.product_id, ce_product.model, ce_product.image, ce_product.price, ce_product.quantity,ce_product.length, ce_product.width, ce_product.height, ce_product.weight, ce_product_description.name, ce_product_description.description, ce_category_description.name as catName FROM ce_product join ce_product_description on ce_product.product_id=ce_product_description.product_id join ce_product_to_category on ce_product_to_category.product_id = ce_product.product_id join ce_category_description on ce_category_description.category_id = ce_product_to_category.category_id  ORDER BY RAND() LIMIT 10;";
